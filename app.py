...
def get_autocomplete_suggestions(search_text: str) -> list[str]:
    """Gets place predictions from Google Places API based on user input."""
    if not search_text: return []
    # FIX: Removed '&components=country:IN' to allow worldwide search.
    url = f"https://maps.googleapis.com/maps/api/place/autocomplete/json?input={search_text}&key={GOOGLE_MAPS_API_KEY}"
    try:
...
def get_route_options(start, end):
    """Fetches alternative driving routes from the Google Directions API."""
    # FIX: Removed '&region=IN' to allow worldwide routing.
    url = f"https://maps.googleapis.com/maps/api/directions/json?origin={start}&destination={end}&key={GOOGLE_MAPS_API_KEY}&alternatives=true"
    try:
...
def get_scenic_spots(route_polyline):
    """Finds highly-rated scenic spots along a route, filtered by relevant types."""
    path = polyline.decode(route_polyline)
    sample_points = [path[int(i * len(path) / 7)] for i in range(7)]
    scenic_places = {}
    keywords = ["scenic lookout", "waterfall", "historic landmark", "temple", "beach", "hiking area", "fort"]
...
@st.cache_data(show_spinner=False)
def get_pit_stops(route_polyline, start_destination):
    """Finds diverse, well-spaced pit stops and calculates the route to them."""
    path = polyline.decode(route_polyline)
    search_points = [
        path[len(path) // 4],
        path[len(path) // 2],
    ...
    for point in search_points:
        lat, lng = point
        search_url = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={lat},{lng}&radius=10000&type=restaurant&keyword=cafe OR restaurant&key={GOOGLE_MAPS_API_KEY}"
        try:
            response = requests.get(search_url, timeout=10)
            results = response.json().get("results", [])
            for place in sorted(results, key=lambda p: p.get('rating', 0), reverse=True)[:2]:
                pit_stop_candidates[place['place_id']] = {
                     "name": place['name'],
            ...
            continue
    detailed_pit_stops = []
    for stop in pit_stop_candidates.values():
        destination_coords = f"{stop['location']['lat']},{stop['location']['lng']}"
        directions_url = f"https://maps.googleapis.com/maps/api/directions/json?origin={start_destination}&destination={destination_coords}&key={GOOGLE_MAPS_API_KEY}"
        try:
            ...
            if route_data.get("routes"):
                ...
                stop['distance_from_start'] = leg['distance']['text']
                stop['duration_from_start'] = leg['duration']['text']
                detailed_pit_stops.append(stop)
        except Exception:
            continue
    return sorted(detailed_pit_stops, key=lambda x: float(x['distance_from_start'].split()[0].replace(',', '')))


@st.cache_data(show_spinner=False)
def create_drive_preview_assets(polyline_str):
    """Creates a looping GIF and a list of 12 static image URLs for the drive preview."""
...
def get_llm_choice(route_options_with_scenery):
    ...
    try:
        ...
        # Robustly extract the first digit from the response
        choice_number_str = next(filter(str.isdigit, response.text), None)
        if choice_number_str:
            return f"Route {int(choice_number_str)}"
        return None
...
def generate_report_html(data):
    gif_base64 = base64.b64encode(data['drive_gif'].getvalue()).decode("utf-8") if data['drive_gif'] else ''
    ...
    pit_stops_html = "".join([f"<li><b>{stop['name']}</b> - ⭐ {stop['rating']} ({stop['total_ratings']} ratings)<br><small>Approx. {stop.get('duration_from_start', '')} ({stop.get('distance_from_start', '')}) from start</small></li>" for stop in data['pit_stops']])
    html_template = f"""
    <!DOCTYPE html><html lang="en"><head><title>Scenic Drive Report</title><style>body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; margin: 0; padding: 0; background-color: #f0f2f6; }}
    ...
    .grid img {{ width: 100%; border-radius: 8px; }} .grid div {{ text-align: center; }} ul {{ padding-left: 0; list-style-type: none;}}
    li {{ margin-bottom: 15px; border-left: 3px solid #007bff; padding-left: 15px;}} small {{ color: #555; }}</style></head><body><div class="container">
...
st.subheader("Plan Your Next Great Drive, Anywhere in the World")
...
with col1:
    start_destination = st_searchbox(get_autocomplete_suggestions, placeholder="Start anywhere...", key="start_searchbox")
with col2:
    end_destination = st_searchbox(get_autocomplete_suggestions, placeholder="Go anywhere...", key="end_searchbox")
...
if st.button("✨ Plan My Scenic Drive", type="primary"):
    if start_destination and end_destination:
        ...
        with st.spinner('Crafting your ultimate scenic drive...'):
            ...
            pit_stops = get_pit_stops(chosen_route['polyline'], start_destination)
...
if st.session_state.report_data:
    ...
    if data['pit_stops']:
        for stop in data['pit_stops']:
            st.markdown(f"**{stop['name']}** - ⭐ {stop.get('rating', 'N/A')}")
            st.caption(f"Approx. {stop.get('duration_from_start', '')} ({stop.get('distance_from_start', '')}) from start")
    else:
